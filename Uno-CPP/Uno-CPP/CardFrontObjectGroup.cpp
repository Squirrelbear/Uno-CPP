...
CardFrontObjectGroup::CardFrontObjectGroup(const int faceValue, const int colourID, const sf::Vector2f& initialPosition, const sf::Font& font)
{
	...
	if (colourID == 4 || label.length() <= 4) {
		...
		DrawableText* shadowText = new DrawableText(sf::Vector2f(0, 0), label, font, fontHeight, sf::Color::Black, sf::Text::Bold);
	...
	size_t fontHeight = (label.length() > 4) ? 10 : 20;
	DrawableText* foregroundText = new DrawableText(sf::Vector2f(0, 0), label, font, fontHeight, textColour, sf::Text::Bold);
	...
	DrawableText* topLeftCorner = new DrawableText(sf::Vector2f(0, 0), cornerLabel, font, fontHeight, sf::Color::White, sf::Text::Bold);
	topLeftCorner->setOffset(sf::Vector2f(5, 5));
	addChild(topLeftCorner);
	DrawableText* bottomRightCorner = new DrawableText(sf::Vector2f(0, 0), cornerLabel, font, fontHeight, sf::Color::White, sf::Text::Bold);
